...
- `noMisplacedAssertion` and `noAwaitInLoops`, only in the standalone smoke script: Node assertions intentionally run outside Vitest, and sequential requests verify session persistence and quota exhaustion.
...
The integration harness uses a temporary local database and removes the remote AI binding and Cloudflare credential environment variables. It never deploys or migrates remote state. Live AI success is not covered; invalid requests and the existing computed fallback need to remain distinct from verified remote model output. Browser screenshots/traces are retained locally on failure in `test-results/`.
