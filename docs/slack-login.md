...
AriadneOS requires Slack login via Better Auth 1.7.4. Hono handles `/api/auth/*`; all other APIs except `/api/health` and the signed Slack event receiver require a verified session. D1 stores auth records in `auth_*` tables. Sign-in also establishes data tenancy: the Slack OIDC profile carries the signing user's workspace, which is stored on the user record as `slackTeamId` and scopes every subsequent request. A session without a workspace is refused rather than falling back to a shared scope.
...
6. Verify login, cancellation, reload persistence, sign out, and rejected unauthenticated API access in staging. `npm run test:smoke -- https://staging.ariadneos.com` checks health and rejected access; supply `ARIADNE_TEST_COOKIE` privately for authenticated checks with a fresh test account.
...
Storage is an append-only observation log partitioned by team, channel, and message timestamp, with a unique `(team_id, event_id)` retry key. It preserves new messages, replies, bot messages, edits and deletion observations as delivered, including their event JSON. Original text remains in earlier records after a deletion event; this is an observation history, not a current-state mirror. File metadata in delivered events is retained; file contents are not downloaded. Observations for an enabled channel are segmented into process sessions, queued, and extracted into steps with message citations. A channel the workspace has not enabled, or a workspace with no install, stores the raw event and derives nothing. Read APIs expose stored Slack data only to a session from the workspace that produced it.
