...
def check(base=None, event=None, all_records=False):
    ...
    if all_records:
        ...
        deleted = git(
            "diff", "--name-only", "--diff-filter=D", "--no-renames", "-z", merge_base
        ).split("\0")
        if any(is_record(name) for name in deleted):
            errors.append(
                "Keep historical work items; add a superseding record instead of deleting or renaming."
            )
        if not records:
            errors.append(
                "Add or update a dated docs/work-items/*.md record (stage new files locally)."
            )
    for name in records:
    ...
    if event:
        ...
        if pr is not None:
            ...
            if not any(
                target == name or target == "./" + name or target.endswith("/" + name)
                for name in records
