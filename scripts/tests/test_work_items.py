...
SPEC = importlib.util.spec_from_file_location('gate', Path(__file__).parents[1] / 'check_work_items.py')
...
class GateTests(unittest.TestCase):
    ...
    def test_valid_record_and_link(self):
        self.record()
        Path('event.json').write_text(json.dumps({'pull_request': {'body': f'[Work item]({RECORD})'}}))
    ...
    def test_incomplete_and_placeholder_records_fail(self):
        for text in (VALID.replace('Status: in-review', 'Status: whatever'),
